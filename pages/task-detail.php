    ...
    <script>
        ...
        function renderDetailChecklist(items, percent, done, total) {
            ...
            c.innerHTML = items.map(item => {
                ...
                // توضیحات آیتم (اگر داشته باشد)
                const MAX_DESC_LEN = 50; // حداکثر طول نمایش کنار عنوان (قابل تغییر)

                // نسخه‌ی امن برای نمایش داخل متن (< > & خنثی می‌شوند)
                const safeDesc = item.description ?
                    String(item.description).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;') :
                    '';
                // نسخه‌ی امن برای صفت title (علاوه بر بالا، " هم خنثی می‌شود)
                const titleDesc = item.description ?
                    String(item.description).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;') :
                    '';

                // اگر طولانی بود، کوتاه کن و «…» بگذار
                const isLongDesc = safeDesc.length > MAX_DESC_LEN;
                const shortDesc = isLongDesc ? safeDesc.slice(0, MAX_DESC_LEN) + '…' : safeDesc;

                // آیکون نشانگر «این آیتم توضیح دارد»
                const descIcon = safeDesc ?
                    `<i class="bi bi-chat-left-text chk-desc-icon" title="${titleDesc}"></i>` :
                    '';
                ...
                return `
                  <div class="checklist-detail-item-wrap">
                    ...
                    <input type="checkbox" ${item.is_done == 1 ? 'checked' : ''}
                           ...
                           ${item.is_done == 1 ? 'title="این آیتم تیک خورده و قابل برداشتن نیست"' : (item.can_toggle_this === false ? 'title="این آیتم به شما ارجاع نشده"' : '')}>
                    <span class="chk-title">${item.title}</span>
                    <span class="chk-desc-zone" id="chk-desc-zone-${item.id}">
                        ${(() => {
                            // اگر کار قفل است (تکمیل/تأیید/متوقف/لغو):
                            if (!checklistCanEdit) {
                                // توضیحات موجود فقط نمایشی؛ آیکونِ افزودن پنهان
                                return safeDesc
                                    ? `<span class="chk-desc-inline" title="${titleDesc}"> — ${shortDesc}</span>`
                                    : '';
                            }
                            // حالت عادی (قابل ویرایش):
                            return safeDesc
                    ...
                    ${mine ? '<span class="badge bg-warning text-dark ms-1">به شما ارجاع شده</span>' : ''}
                    <span class="chk-meta">${item.is_done == 1 && item.done_by_name ? '✓ ' + item.done_by_name.trim() : ''}</span>
                    ${checklistCanEdit ? (item.is_done == 1 ? `
                        <i class="bi bi-lock-fill text-muted"
                           style="cursor:help;"
                           onclick="showLockReason()"
                           title="این آیتم تیک خورده و قفل شده است؛ دیگر قابل تغییر یا حذف نیست."></i>
                    ` : `
                        <button class="btn btn-link btn-sm p-0" onclick="editChecklistItem(${item.id}, '${(item.title || '').replace(/'/g, "\\'")}')">
                            <i class="bi bi-pencil"></i>
                        </button>
                        <button class="btn btn-link btn-sm text-danger p-0" onclick="deleteChecklistItem(${item.id})">
       ...
       function startEditDesc(itemId) {
            // اگر کار قفل است، اجازه‌ی ویرایش توضیحات نده
            if (!checklistCanEdit) { showToast('این کار به پایان رسیده و قابل ویرایش نیست', 'warning'); return; }

            const zone = document.getElementById('chk-desc-zone-' + itemId);
            if (!zone) return;

            // پیدا کردن توضیحات فعلی این آیتم از حافظه
            const item = currentChecklistItems.find(i => i.id == itemId);
            const currentDesc = (item && item.description) ? item.description : '';

            // ساخت textarea + دکمه تیک
            zone.innerHTML = `
                <span class="chk-desc-edit" style="display:inline-flex; align-items:center; gap:4px; vertical-align:middle;">
                    <textarea id="chk-desc-input-${itemId}" class="form-control form-control-sm"
                        rows="1" style="min-width:180px; font-size:0.8rem; display:inline-block; width:350px;"
                        placeholder="توضیحات...">${currentDesc.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</textarea>
                    <button class="btn btn-link btn-sm text-success p-0" title="ثبت توضیحات"
                        onclick="saveEditDesc(${itemId})">
                        <i class="bi bi-check-lg"></i>
                    </button>
                    <button class="btn btn-link btn-sm text-secondary p-0" title="انصراف"
                        onclick="cancelEditDesc(${itemId})">
