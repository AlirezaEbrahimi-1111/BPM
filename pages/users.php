...
if (!$__me || !hasPermission($__me, 'manage_users')) {
...
<head>
    ...
    <style>
        ...
        :root[data-theme="dark"] .bonus-req-box {
            background: var(--surface);
            border-color: var(--border-soft);
        }

        .bonus-req-box .bonus-req-title {
            font-weight: 700;
            font-size: .85rem;
            margin-bottom: .6rem;
            display: flex;
        ...
        .bonus-req-row {
            ...
            flex-wrap: wrap;
            padding: .5rem 0;
            border-top: 1px dashed #e5c979;
            font-size: .82rem;
        }

        :root[data-theme="dark"] .bonus-req-row {
            border-top-color: var(--border-soft);
        }

        .bonus-req-row:first-of-type {
            border-top: none;
        }

        .bonus-req-row .bonus-req-info {
            display: flex;
            flex-direction: column;
            gap: .15rem;
        }

        .bonus-req-row .bonus-req-note {
            color: #6b7280;
            font-size: .76rem;
        }
...
<body>
                    ...
                    <form id="createUserForm" novalidate>
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select" id="c_activity_section"></select>
                            </div>
                            <div class="col-md-6">
                ...
                <div class="modal-body pt-0">
                    ...
                    <div id="tab-info" class="tab-pane">
                        <div class="row g-3 pt-3">
                            ...
                            <div class="col-12">
                                <label class="form-label">واحدها <small class="text-muted">(روی ⭐ بزنید تا واحد اصلی شود)</small></label>
                                <input type="hidden" id="e_activity_section">
                                <div id="secPicker" class="sec-picker">
                                    <div id="secTags" class="sec-tags"></div>
                                    <div class="sec-picker-input" onclick="toggleSecDropdown(event)">
                                        <span id="secPlaceholder" class="text-muted">افزودن واحد…</span>
                                        <i class="bi bi-chevron-down"></i>
                                    </div>
                                    <div id="secDropdown" class="sec-dropdown" style="display:none;"></div>
                                </div>
                            </div>
                            <div class="col-12">
                    ...
                    <div id="tab-shift" class="tab-pane" style="display:none">
                        <div class="row g-3 pt-3">
                            ...
                            <div class="col-12">
                                <hr class="my-1"><strong class="small">شیفت اول</strong>
                            </div>
                            <div class="col-md-6">
                            ...
                            <div id="shift2Block" style="display:none" class="col-12">
                                <div class="row g-3">
                                    <div class="col-12">
                                        <hr class="my-1"><strong class="small">شیفت دوم</strong>
                                    </div>
                                    <div class="col-md-6">
                                        <label class="form-label">شروع</label>
                            ...
                            <div class="col-12">
                                <hr class="my-1"><strong class="small">اطلاعات مالی</strong>
                            </div>
                            <div class="col-md-6">
    ...
    <script>
        ...
        function renderSecTags() {
            ...
            const ph = document.getElementById('secPlaceholder');
            if (!tags) return;
            if (!secPrimary && secList.length) secPrimary = secList[0];
            syncPrimaryInput();
            let html = '';
            secList.forEach(k => {
                const isPrimary = (k === secPrimary);
                const star = isPrimary ?
                    `<span class="star on" title="واحد اصلی">★</span>` :
                    `<span class="star" title="تعیین به‌عنوان اصلی" onclick="setPrimarySec('${k}')">☆</span>`;
                html += `<span class="sec-tag ${isPrimary ? 'is-primary' : ''}">${star}<span>${secLabel(k)}</span><span class="x" title="حذف" onclick="removeSec('${k}')">×</span></span>`;
            });
            tags.innerHTML = html;
            if (ph) ph.style.display = secList.length ? 'none' : 'inline';
        }

        function syncPrimaryInput() {
            const el = document.getElementById('e_activity_section');
            if (el) el.value = secPrimary || '';
        }

        function secLabel(key) {
            const s = orgSections.find(x => x.section_key === key);
            return s ? s.section_label : key;
        }
        ...
        function buildSecDropdown() {
            const dd = document.getElementById('secDropdown');
            const html = orgSections
                .filter(s => !['management', 'supervisor'].includes(s.section_key))
                .map(s => {
                    const isPicked = secList.includes(s.section_key);
                    return `<div class="sec-option ${isPicked ? 'picked' : ''}" ${isPicked ? '' : `onclick="addSec('${s.section_key}')"`}>${s.section_label}</div>`;
                }).join('');
            dd.innerHTML = html || '<div class="sec-option picked">واحدی نیست</div>';
        }

        function addSec(key) {
            if (!secList.includes(key)) secList.push(key);
            if (!secPrimary) secPrimary = key;
            renderSecTags();
            buildSecDropdown();
        }

        function removeSec(key) {
            secList = secList.filter(k => k !== key);
            if (secPrimary === key) secPrimary = secList[0] || '';
            renderSecTags();
            buildSecDropdown();
        }

        function setPrimarySec(key) {
            if (secList.includes(key)) secPrimary = key;
            renderSecTags();
        }
        document.addEventListener('click', (e) => {
            const picker = document.getElementById('secPicker');
        ...
        document.addEventListener('DOMContentLoaded', async () => {
            ...
            try {
                const r = await fetch('/api/admin/me.php', {
                    headers: ah()
                });
        ...
        // پر کردن یک <select> واحد با گزینه‌ها
        function fillSectionSelect(selectId, selectedValue = '') {
            const el = document.getElementById(selectId);
            if (el) el.innerHTML = buildSectionOptions(selectedValue);
        }
            ...
            const colDefs = [{
                ...
                },
                {
                    ...
                    cellRenderer: p => p.value == 1 ?
                        '<span class="status-badge status-active">فعال</span>' : '<span class="status-badge status-inactive">غیرفعال</span>'
                },
                {
                    field: 'last_login',
    ...
    <button class="ag-action-btn" style="color:#dc2626" title="حذف کاربر"
        ...
        function grantLeaveBonus(userId, name) {
            fetch('/api/attendance/leave-balance.php?user_id=' + userId, { headers: ah() })
                .then(r => r.json())
                .then(d => {
        ...
        async function loadUsers() {
            ...
            try {
                ...
                showAlert('خطا در ارتباط بییییییا سرور', 'danger');
            } finally {
        ...
        async function openCreateModal() {
            ...
            try {
                const r = await fetch('/api/admin/check-user-limit.php', {
                    headers: ah()
                });
                const d = await r.json();
                if (!d.allowed) {
        ...
        async function createUser() {
            ...
            if (!fn || !ln) {
                showCreateAlert('نام و نام خانوادگی اجباری است');
                return;
            }
            if (!/^09[0-9]{9}$/.test(phone)) {
                showCreateAlert('موبایل باید 09xxxxxxxxx باشد');
                return;
            }
            if (pw.length < 8) {
                showCreateAlert('رمز عبور حداقل ۸ کاراکتر');
                return;
            }
            if (!/[A-Za-z]/.test(pw)) {
                showCreateAlert('رمز عبور باید حداقل یک حرف انگلیسی داشته باشد');
                return;
            }
            if (!/[0-9]/.test(pw)) {
                showCreateAlert('رمز عبور باید حداقل یک عدد داشته باشد');
                return;
            }
            if (pw !== pw2) {
                showCreateAlert('رمز عبور و تکرار یکسان نیستند');
                return;
            }
        ...
        async function openEditModal(userId) {
            ...
            document.getElementById('editModalName').textContent = [u.first_name, u.last_name].filter(Boolean).join(' ') || u.phone;
            ...
            try {
                const rs = await fetch('/api/admin/user-sections.php?user_id=' + userId, {
                    headers: ahj()
                });
                const ds = await rs.json();
                if (ds.success && Array.isArray(ds.sections)) {
                    secList = ds.sections.slice();
                    secPrimary = ds.primary || u.activity_section || secList[0] || '';
                }
            } catch {}
        ...
        async function saveUser() {
            ...
            if (!/^09[0-9]{9}$/.test(ph)) {
                switchTab('info', document.querySelector('.modal-tab'));
                showModalAlert('موبایل نامعتبر است');
                return;
            }
            if (pw && pw !== pw2) {
            ...
            const units = UNIT_LIST
                ...
                .map(c => ({
                    activity_unit: c,
                    is_primary: document.getElementById(`primary_${c}`)?.checked ? 1 : 0
                }));
            ...
            const sectionsChanged = collectSections().slice().sort().join(',') !== originalSections;
            const hasChanged = sectionsChanged || Object.keys(currentData).some(k => {
