...
class LeaveManager
{
    ...
    {
        // وضعیت را به تأیید شده تغییر بده — فقط اگر قبلا نهایی نشده باشد.
        // شرط status <> 'approved' یک قفل ایمنی است: اگر دو درخواست تأیید
        // هم‌زمان (دابل‌کلیک یا دو تأییدکننده) هر دو به این‌جا برسند، فقط
        // اولی rowCount()=1 می‌گیرد و بقیه no-op می‌شوند؛ در غیر این صورت
        // «used_leave = used_leave + ?» دو بار اجرا شده و موجودی مرخصی
