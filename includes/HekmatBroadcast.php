 ...
 * ─────────────────────────────────────────────────────────────────
 * منطق مشترک ارسال روزانه‌ی حکمت‌های نهج‌البلاغه — هم توسط
 * cron/hekmat_daily.php (اجرای خودکار) و هم توسط API صفحه‌ی مدیریت
 * (پیش‌نمایش، ارسال آزمایشی، ارسال دستی) استفاده می‌شه — تا منطق
 * انتخاب جمله/ساخت پیام فقط یک‌جا نوشته بشه.
 *
 * 🔒 ارسال واقعی پیامک از طریق همون کلاس SMS موجود (includes/sms.php،
 * پنل ملی‌پیامک) انجام می‌شه. چون sms_logs.user_id ستون NOT NULL داره و
 * این گیرنده‌ها لزوما کاربر ثبت‌شده‌ی سیستم نیستن، user_id=1
 * (سوپرادمین/مالک سیستم) به‌عنوان صاحب این ارسال‌های خودکار پاس داده می‌شه.
 *
 * 🔒 فیلتر محتوایی خط پیامک (کلمات حساس): موقع تست این فیچر معلوم شد
 * پنل ملی‌پیامک بعضی کلمات سیاسی/حساس (مثل «فتنه») رو به‌صورت کامل رد
 * می‌کنه (RetStatus=35/InvalidData) — حتی وقتی جمله‌ی دینی کاملا عادیه.
 * چون نهج‌البلاغه پر از همچین کلماتیه، درست قبل از ارسال واقعی (نه توی
 * چیزی که ذخیره/نمایش داده می‌شه)، این کلمات با یه zero-width space
 * (U+200B) وسطشون جایگزین می‌شن — از نظر چشم نامرئیه، ولی دیگه substring
 * دقیقی که فیلتر دنبالشه پیدا نمی‌شه. لیست زیر تجربی/دستیه: هر کلمه‌ی
 * جدیدی که رد شد (توی تاریخچه‌ی ارسال با status=failed دیده می‌شه)، باید
...
class HekmatBroadcast
{
    ...
    /** کلماتی که پنل پیامک رد می‌کنه — دستی و تجربی، بر اساس موارد کشف‌شده.
     *  هر دو رسم‌الخط کلاسیک (ى/ي عربی) و استاندارد فارسی (ی) اضافه شده
     *  چون فیلتر روی نسخه‌ی دقیق حروف حساسه، نه معنی. */
    ...
    /** جایگزینی کامل لیست جملات — هر خط یک جمله؛ خطوط خالی نادیده گرفته می‌شن */
    public static function replaceQuotes(PDO $db, string $rawText): int
    {
        ...
        try {
            ...
            // 🔒 چون لیست عوض شده، اندیس چرخش قبلی دیگه معنی نداره —
            // برمی‌گرده به اول لیست تا خارج از محدوده نره
    ...
    /** جایگزینی کامل لیست شماره‌ها — هر خط یک شماره؛ نرمال‌سازی + حذف تکراری */
    ...
    /**
     * جمله‌ای که «اگر همین الان بفرستیم» انتخاب می‌شه — بدون تغییر
     * next_index (صرفا برای پیش‌نمایش/تست بدون اثر جانبی)
     */
    ...
    /** پیش‌نمایش کامل پیامی که امروز/الان قراره فرستاده بشه */
    ...
    /**
     * اجرای واقعی ارسال روزانه — به همه‌ی گیرنده‌های فعال، با جمله‌ی
     * بعدی چرخش. هم توسط cron و هم توسط دکمه‌ی «ارسال دستی الان»
    ...
    /** ارسال آزمایشی به یک شماره‌ی دلخواه — تأثیری روی چرخش/تاریخ آخرین ارسال نداره */
    ...
    /**
     * پیامک هشدار به مدیر وقتی ارسال روزانه (کامل یا بخشی‌ازش) ناموفق
     * بود — best-effort: اگه خود این پیامک هشدار هم ناموفق بشه، فقط لاگ
     * می‌شه، هیچ استثنایی به بیرون درز نمی‌کنه تا جریان اصلی ارسال رو خراب نکنه
     */
    ...
    /** آخرین علت خطای ثبت‌شده در sms_logs — برای نشون‌دادن دلیل واقعی ناموفقی، نه فقط یه پیام کلی */
    ...
    /**
     * جایگزینی کلمات فیلترشده با نسخه‌ی نامرئی‌شده (zero-width space وسط
     * کلمه) — فقط روی متنی که واقعا به API فرستاده می‌شه، نه چیزی که
    ...
    /** نسخه‌ی مستقل از normalizePhone توی sms.php — عمدا کپی شده تا اون فایل مشترک دست‌نخورده بمونه */
    private static function normalizePhone(string $phone): ?string
    {
        ...
        if (!preg_match('/^9\d{9}$/', $phone)) {
            return null; // فرمت موبایل ایران نیست
        }

        return '0' . $phone; // ذخیره‌ی یکنواخت به‌شکل 09xxxxxxxxx
    }
}
