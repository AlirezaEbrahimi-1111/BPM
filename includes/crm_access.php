<?php
/**
 * دسترسی موقت ماژول «فروش / فاکتور».
 *
 * تا وقتی این ماژول در حال ساخت است، فقط افراد زیر منو و صفحه‌ها را می‌بینند:
 *   - کاربر id = 1
 *   - کاربر id = 19 (رضا فضایلی)
 *   - کل تیم حسابداری سازمان ۱  →  هر کاربر فعالی که «بخش فعالیتش»
 ...
 *      «واحد/unit»؛ ستون users.activity_unit عملا برای همه 'all' است.
 *      چک قدیمی 'AC' هم نگه داشته شده تا اگر جایی از آن استفاده شد نشکند.)
 *   - شماره‌موبایل‌های موقت فهرست $EXTRA_PHONES
 *
 * این «دیدن» است، نه «نوشتن» — نوشتن هنوز به مجوز
 * users.is_create_official_invoice / is_sales_manager وابسته است
 * (هم در PHP هم در سرویس Go).
 *
 * برای افزودن/حذف یک نفر خاص: id را در آرایه‌ی $ids یا موبایل را در
 * $EXTRA_PHONES عوض کن.
 */
...
function crmModuleUserIds(PDO $db): array
{
    ...
    if ($EXTRA_PHONES) {
        try {
            ...
            // اگر جدول/ستون در دسترس نبود، فقط id‌های ثابت می‌مانند.
        }
    }

    // کل تیم حسابداری سازمان ۱ — بر پایهٔ «بخش فعالیت» (نه واحد).
    try {
        $st = $db->query("
            SELECT u.id
            FROM users u
...
/**
 * دسترسی «گزارش همکاران» (pages/inv-partner-report.php).
 *
 ...
 * ظاهر نشود (به آن با لینک مستقیم می‌رسند). معادل requireReportAccess در
