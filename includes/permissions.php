 ...
 *    قبلا `$user_id === 1` در چند فایل پراکنده بود — همه حذف شد.
 */
...
function getPermissionMatrix(): array
{
    $supervisorPermissions = [
        ...
        'grant_user_permissions',   // اجازهٔ دادن اختیار فردی به دیگران
    ];
    ...
    return [
        ...
        // ── مقدار قدیمی role — همیشه معادل supervisor رفتار
        ...
        // ⚠️ 'manage_users' اینجا فقط یک مجوز «درشت» است؛ محدودشدن
        //    به زیرمجموعه از طریق canManageTargetUser() انجام می‌شود،
        //    نه این جدول. هرجا این اجازه استفاده می‌شود، باید حتما
        //    با canManageTargetUser() ترکیب شود، نه isSameOrganization().
        //
        // ⚠️ 'view_org_dashboard_reports' عمدا به‌جای 'view_all_org_tasks'/
        //    'monitor_all_workflows' اضافه شد: اون دو مجوز به چند endpoint
        //    دیگه هم دسترسی می‌دن که برای نقش مدیر مناسب نیستن (مثلا
        //    api/admin/routines-all.php که فیلتر organization_id نداره و
        //    دادهٔ همهٔ سازمان‌ها رو برمی‌گردونه). این مجوز جدید فقط
        //    گزارش‌های داشبورد (کارهای واگذارشده‌ی تأخیردار + گلوگاه‌ها) رو
        //    برای کل سازمان خود مدیر باز می‌کنه، نه چیز دیگه‌ای.
        'manager' => [
            'manage_users',
            'view_section_tasks',
 ...
 *
 * ⚠️ حتما از این تابع استفاده کنید. اگر کوئری دستی بنویسید و
 *    ستونی (مثلا can_create_workflow) را جا بیندازید، تصمیم اشتباه
 *    گرفته می‌شود.
 */
...
function hasPermission(?array $user, string $permission): bool
{
    ...
    // ── لایهٔ ۲: اختیار فردی ────────────────────────────
    // اگر مدیر سازمان صریحا این اجازه را به کاربر داده باشد،
    // بر نقشش اولویت دارد.
...
/**
 * آیا این کاربر در سطح سازمانی (supervisor/admin/سوپرادمین) است؟
 *
 * برای اقداماتی که عمدا به «manager» داده نمی‌شود (مثلا بازگردانی
 * کاربر حذف‌شده — یک عملیات حساس‌تر از مدیریت روزمرهٔ زیرمجموعه).
 * برخلاف manage_users در جدول اجازه‌ها (که manager هم دارد)، این
 * تابع صرفا «همان‌قدیمی admin/supervisor» را برمی‌گرداند.
 */
...
/**
 * شناسهٔ همهٔ زیردستان یک مدیر — با هر عمقی (زنجیرهٔ کامل manager_id)،
 * نه فقط زیردستان مستقیم.
 *
 * ⚠️ محافظ حلقه: اگر دادهٔ manager_id به‌اشتباه حلقه بسازد
 *    (مثلا A مدیر B و B مدیر A)، با مجموعهٔ visited و سقف ۵۰۰
 *    مرحله، هرگز در حلقهٔ بی‌نهایت گیر نمی‌افتد.
 *
 * ⚠️ مرز سازمان: حتی اگر دادهٔ manager_id به‌اشتباه به کاربر
 *    سازمان دیگری اشاره کند، این تابع هرگز از مرز سازمان خود
 *    مدیر عبور نمی‌کند (شرط organization_id در هر کوئری).
 *
 * @return int[] شناسه‌های زیردستان (بدون خود مدیر)
 */
...
/**
 * آیا کاربر جاری اجازه دارد روی کاربر هدف عملیات مدیریتی
 * (ویرایش/حذف/فعال‌سازی/تغییر واحد و ...) انجام دهد؟
 *
 * قاعده:
