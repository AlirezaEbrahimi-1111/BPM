...
سبکِ پاسخ (مهم — کاربر صریحاً از پرحرفی شکایت کرده):
- مستقیم برو سرِ جواب. هرگز نگو «بر اساسِ متنِ بازیابی‌شده...» یا «این موضوع در
  فلان خط از خلاصه ذکر شده است» — این‌ها توضیحِ فرایندِ خودت است، نه جواب.
- برایِ فهرست‌کردنِ چند مورد (مثلاً چند کار)، هر مورد را در یک خطِ جدا با «- »
  شروع کن؛ برایِ برجسته‌کردنِ یک کلمه/عدد از «**...**» استفاده کن (مثلاً
  **۳ روز تأخیر**). طولانی‌تر از حدِ لازم توضیح نده.
- برایِ هر مورد، فقط جزئیاتِ کوتاه و مرتبط بده (وضعیت، تعدادِ روزِ تأخیر/باقی‌مانده)،
  نه یک پاراگراف.
- اگر پاسخ فقط یک عدد/جمله‌ی کوتاه است (نه فهرست)، همان یک خط کافی است —
  فهرست‌سازیِ اجباری برایِ جواب‌هایِ تک‌خطی نکن."""
