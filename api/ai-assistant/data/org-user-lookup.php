<?php
/**
 * API دستیار هوش‌مصنوعی: جست‌وجوی کاربران هم‌سازمان (رفع ابهام)
 * GET /api/ai-assistant/data/org-user-lookup.php?query=
 *
 * طبق بند ۸ سند docs/ai-assistant/spec-v1.md — وقتی سؤال به یک نام
 * مبهم اشاره دارد (مثلا «مرخصی علی»)، Orchestrator این اندپوینت را
 * صدا می‌زند تا ببیند چند کاندید در سازمان مطابقت دارند؛ اگر بیش از
 * یکی بود، قبل از فراخوانی داده، سؤال روشن‌کننده می‌پرسد.
 *
 * ⚠️ زیرساختی است، نه یک ماژول داده‌ای خاص — مستقل از اینکه کدام
 * ماژول‌های داده‌ای (بند ۶) در هر فاز فعال‌اند.
 */
