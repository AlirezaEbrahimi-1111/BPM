<?php
/**
 * API دستیار هوش‌مصنوعی: خلاصه‌ی کارهای یک کاربر + فهرست ریز کارها
 * GET /api/ai-assistant/data/task-summary.php?user_id=
 *
 * طبق بند ۶ سند docs/ai-assistant/spec-v1.md — یکی از فهرست ثابت
 * اندپوینت‌های خواندنی Orchestrator (نه یک ابزار عمومی/Agent). دقیقا
 * همان کلاس Auth و همان منطق آمار (TaskManager::getTaskStats) که بقیه‌ی
 * BPM استفاده می‌کند را به‌کار می‌برد — بدون منطق موازی.
 *
 * ⚠️ در استقرار نهایی، این مسیر فقط از سمت ai-service (سرور-به-سرور)
 * فراخوانی می‌شود، نه از مرورگر — ولی کنترل دسترسی زیر (JWT + RBAC)
 * صرف‌نظر از فراخواننده، همیشه برقرار است.
 *
 * user_id (اختیاری): اگر خالی باشد یا برابر کاربر جاری باشد، خلاصه‌ی
 * خود کاربر برمی‌گردد. اگر کاربر دیگری از همان سازمان درخواست شود،
 * فقط مدیر/سرپرست مجاز است (همان قاعده‌ی api/tasks/overview.php).
 *
 * علاوه‌بر آمار کلی، فهرست ریز کارها (`tasks`) هم برمی‌گردد — چون
 * سؤالاتی مثل «کدام کارها مال من است» یا «جزئیات کار X چیست» به
 * چیزی بیش از یک شمارش ساده نیاز دارند. قاعده‌ی دقیق اینکه چه کاری
 * برای کدام کاربر نمایش داده می‌شود (طبق تصمیم صریح کاربر):
 *   ۱) مدیر/سرپرست همیشه اجازه‌ی دیدن خلاصه‌ی هرکسی در سازمان خودش را دارد
 *   ۲) تعریف‌کننده‌ی کار همیشه در فهرستش می‌بیند
 *   ۳) ارجاع‌دهنده تا وقتی کار به وضعیت پایانی نرسیده در فهرستش می‌بیند
 *      (چون فیلد صریح «تاریخ بازگشت» وجود ندارد، این تفسیر عملی
 *      «تا وقتی برمی‌گردد» است — از task_history با action='delegated')
 *   ۴) مسئول انجام *فعلی* (assignee_id) در فهرستش می‌بیند
 *   ۵) فقط کارهای همان سازمان — هرگز سازمان دیگر
 *   ۶) کارهای حذف‌شده (is_deleted=1) تحت هیچ شرایطی نمایش داده نمی‌شوند
 * برای کنترل حجم/لتنسی (بند ۱۵)، حداکثر ۵۰ مورد اخیر برمی‌گردد؛ فاز
 * بعد می‌تواند فیلتر/صفحه‌بندی اضافه کند.
 *
 * نکته‌ی «کارهای امروز»: طبق تعریف صریح سازمان، «امروز» یعنی امروز +
 * تمام کارهای معوقه (نه فقط due_date === امروز). به همین دلیل، به‌جای
 * تکیه بر `stats.today` (که از منطق عمومی TaskManager می‌آید و این
 * تعریف را نمی‌داند)، فیلد `due_today_or_earlier_count` را هم اضافه
 * کردیم — محاسبه‌شده در همین فایل، مخصوص دستیار.
 */
...
try {
    ...
    if ($targetUserId !== $requester_id) {
        // 🔒 فقط مدیر/سرپرست اجازه دارد خلاصه‌ی کارهای فرد دیگری را ببیند —
    ...
    // ─── قاعده‌ی دسترسی ریز کارها (طبق تصمیم صریح کاربر) ───
    // ۱) مدیر همیشه دسترسی دارد → همین بالاتر، در گیت requester/target اعمال شد
    ...
    //    چون هیچ فیلد صریح «بازگشت» وجود ندارد، این را این‌طور تفسیر کردیم:
    //    تا وقتی که وضعیت کار به حالت پایانی (تکمیل/رد/توقف/پایان دوره) نرسیده
    // ۴) مسئول انجام (assignee_id) — چون این ستون فقط مقدار *فعلی* را نگه
    //    می‌دارد (نه تاریخچه)، همین «assignee_id = هدف» خودش دقیقا همان قاعده‌ی
    //    «فقط وقتی که الان مسئول انجام است» را برآورده می‌کند
    // ۵) فقط سازمان خودش — با organization_id در WHERE، نه فقط با اعتماد به تطبیق کاربر
    // ۶) کارهای حذف‌شده هرگز نمایش داده نمی‌شوند — is_deleted = 0 (بدون استثنا)
    $terminalStatuses = "'completed', 'rejected', 'stopped', 'period_done'";
    // 🔒 ستون‌ها و JOINهای dr/ph دقیقا همون‌هایی هستن که api/tasks/my-tasks.php
    // برای تشخیص «در انتظار تأیید من» / «در انتظار تمدید من» استفاده می‌کنه —
    // بدون این‌ها، taskIsOverdue()/taskIsDueToday() نمی‌تونن این دو حالت خاص
    ...
    // برای enrichTaskDates()/pe_state() — همون الگوی بقیه‌ی ماژول کارها
    // (بدون organization_id، طبق رفتار فعلی سراسری این ماژول)
    $holidays = getHolidaySet($db);

    // ─── فهرست کارهایی که هدف در آن‌ها هنوز «ارجاع‌دهنده‌ی فعال» است (برای برچسب نقش) ───
    $stmt = $db->prepare("
    ...
    // 🔒 معوقه/امروز/برچسب وضعیت — دیگه این‌جا دوباره و ساده‌شده محاسبه
    // نمی‌شن؛ همون طبقه‌بندی‌کننده‌ی کاننیک includes/task-status-helper.php
    // صدا زده می‌شه (که خودش دقیقا هم‌معنی TF.isOverdue/TF.isDueToday در
    // assets/js/task-filters.js است — یعنی دستیار دقیقا همون چیزی رو
    // می‌بینه که خود کاربر روی داشبورد می‌بینه، نه یک تفسیر ساده‌شده‌ی
    // جداگانه). تاریخ عمدا سمت PHP محاسبه می‌شه، نه به‌عهده‌ی LLM.
    $today = date('Y-m-d');
    ...
    $tasks = array_map(function ($t) use ($targetUserId, $activeReferrerTaskIds, $today, $db, $holidays, &$dueTodayOrEarlierCount) {
        ...
        // next_due_date/overdue_periods (برای کار دوره‌ای) از همین‌جا میان —
        // همون موتور مشترکی که api/tasks/detail.php و بقیه‌ی صفحات استفاده می‌کنن
    ...
    // آمار «معوقه/امروز» برای خلاصه‌ی کلی، از همین فهرست جمع زده می‌شه (نه
    // از TaskManager::getTaskStats) — تا خلاصه و ریز کارها همیشه با هم
    // بخونن؛ getTaskStats قاعده‌ی متفاوتی برای «معوقه» داره (کارهای
    // فرآیندی رو نمی‌بینه، تمدید مهلت رو لحاظ نمی‌کنه) که فعلا دست‌نخورده
    // مونده چون جای دیگه هم استفاده می‌شه
    ...
    echo json_encode([
        ...
        'stats'   => $stats,
        // طبق تعریف سازمان: امروز + معوقه = «بایدهای امروز» — جایگزین
        // stats.today (که فقط due_date دقیقا امروز را می‌شمارد و باعث
        // پاسخ ناقص می‌شد)
        'due_today_or_earlier_count' => $dueTodayOrEarlierCount,
        // 🆕 معوقه‌ی دقیق و هم‌خوان با فهرست ریز زیر — به‌جای stats.overdue
