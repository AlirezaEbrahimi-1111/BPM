<?php
/**
 * API: جستجوی سراسری در متن پیام‌های همه‌ی گفتگوهای کاربر جاری
...
try {
    ...
    // 🔒 قبلا «طرف مقابل» بدون توجه به نوع گفتگو join می‌شد — برای گروه
    // (چند نفره)، این یعنی هر پیام به‌ازای هر عضو دیگه یک‌بار تکراری
    // برمی‌گشت (مثلا یه پیام در گروه ۷نفره → ۶ ردیف عینا تکراری)، که
    // LIMIT رو خیلی زود با تکرارهای یک پیام پر می‌کرد و نتایج واقعی
    // دیگه (به‌خصوص قدیمی‌ترها) اصلا به کاربر نمی‌رسید — دقیقا همون‌چیزی
    // که کاربر «جستجو چیزی پیدا نمی‌کنه» گزارش کرد. الان «طرف مقابل» فقط
    // برای گفتگوی direct (که واقعا یک‌نفره‌ست) join می‌شه؛ برای
    // گروه/کانال، عنوان خود گفتگو (c.title) استفاده می‌شه.
    $sql = "
