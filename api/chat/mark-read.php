<?php
/**
 * API: علامت‌گذاری خوانده‌شدن پیام‌ها تا یک نقطه‌ی مشخص
 * POST /api/chat/mark-read.php   body: { conversation_id, up_to_id? }
 * اگر up_to_id داده نشود، کل گفتگو تا آخرین پیام خوانده‌شده علامت می‌خورد
 * (رفتار قدیمی — برای «علامت‌گذاری همه به‌عنوان خوانده‌شده»)؛ اگر داده شود،
 * فقط تا همان پیام جلو می‌رود (برای پیشرفت تدریجی خواندن هنگام اسکرول).
 */
...
try {
    ...
    // 🔒 فقط شرکت‌کننده‌های همین گفتگو اجازه‌ی علامت‌زدن خوانده‌شده را دارند
    ...
    // up_to_id هرگز نباید از آخرین پیام واقعی گفتگو جلوتر برود
