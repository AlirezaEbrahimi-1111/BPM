...
try {
    ...
    if ($role === 'admin') {
        ...
        if (!chatUserIsGroupManager($db, $conversationId, $user_id)) {
            ...
            echo json_encode(['success' => false, 'message' => 'فقط مدیر گروه می‌تواند عضو دیگری را مدیر کند']);
            exit;
        }
    } else {
        // 🔒 عزل از مدیریت: فقط سازنده
        if (!chatUserIsGroupCreator($db, $conversationId, $user_id)) {
            http_response_code(403);
            error_log("Chat set-member-role (demote) denied | user_id={$user_id} | conversation_id={$conversationId}");
            echo json_encode(['success' => false, 'message' => 'فقط سازنده‌ی گروه می‌تواند یک مدیر را عزل کند']);
            exit;
        }
    }
    ...
    error_log("Chat set-member-role error: " . $e->getMessage());
}
