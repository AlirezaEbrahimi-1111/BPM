...
// اگر درخواست‌دهنده خود تعریف‌کننده باشد → تأیید خودکار
// ==================================================
...
try {
    ...
    // manager فقط اگر سازنده/مسئول این کار زیرمجموعهٔ خودش باشد
    if (!$isAssignee && !$isCreator && !$isSection) {
    ...
    // ✅ تأیید خودکار اگر درخواست‌دهنده خود تعریف‌کننده باشد
    if ($isCreator) {
        // ⚠️ last_approved_date این‌جا ست نمی‌شود — رفع معوقه = بخشش دوره‌های
        //    گذشته، نه تأیید دورهٔ امروز. نوشتن CURDATE() این‌جا باعث می‌شد
        //    گارد فرانت (isLastApprovedDateToday) دکمهٔ «تکمیل» را تا آخر روز
