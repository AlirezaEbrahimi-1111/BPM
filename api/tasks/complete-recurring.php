...
try {
    ...
    // 🔒 کنترل دسترسی — قبلا هیچ چکی نبود: هر کاربر لاگین‌شده (حتی از
    // سازمان دیگر) می‌توانست با حدس task_id، دورهٔ کار پیوستهٔ دیگران را
    // «تکمیل‌شده» علامت بزند. taskUserAccess فقط سازنده/مسئول فعلی/مدیر
    // هم‌سازمان آن‌ها را می‌پذیرد؛ بیننده و مسئول صرف چک‌لیست حق تکمیل ندارند.
    ...
    // 🔒 ریست چک‌لیست برای دوره‌ی بعدی — قبلا این‌جا فراموش شده بود (فقط
    // مسیر قدیمی registerRecurringPeriod در api/checklist/_helpers.php این
    // ریست رو داشت، ولی اون تابع الان جایی صدا زده نمی‌شه)؛ بدون این خط،
    // تیک‌های دوره‌ی قبل تا ابد روی چک‌لیست دوره‌ی جدید هم باقی می‌موندن
    $db->prepare("
