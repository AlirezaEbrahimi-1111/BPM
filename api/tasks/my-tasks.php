...
try {
    ...
    // ════════════════════════════════════════════════════════════
    //  مسیر بایگانی چک‌لیست
    //  تسک‌هایی که کاربر «صرفا چک‌لیستی» است و همه‌ی آیتم‌های
    ...
    if ($filter === 'checklist_archive') {
        ...
        $archiveSql = "
            ...
            WHERE t.is_deleted = 0
              ...
              -- ولی هیچ آیتم ناتمامی برای کاربر/واحدش باقی نمانده باشد
              AND NOT EXISTS (
...
";
    ...
    // 🆕 پیش‌واکشی دسته‌ای تاریخ‌های تکمیل برای همهٔ تسک‌های دوره‌ای —
    ...
    foreach ($all_tasks as $task) {
        // 🔒 رفع ناهماهنگی: این برگشت‌از period_done به دوره‌ی بعدی قبلا فقط
        // در overview.php/delegated-tasks.php انجام می‌شد، نه اینجا — یعنی
        // وضعیت یک کار تکرارشونده بسته به اینکه کاربر کدوم صفحه را باز
