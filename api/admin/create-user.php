...
try {
    ...
    if ($current_org_id) {
        // 🔒 قفل هم‌زمانی برای این سازمان: بدون آن، دو درخواست «افزودن
        // کاربر» هم‌زمان هر دو COUNT را زیر سقف می‌بینند و هر دو کاربر
        // ساخته می‌شود (عبور از سقف اشتراک). قفل با پایان درخواست آزاد می‌شود.
        $seatLock = $db->prepare('SELECT GET_LOCK(?, 10)');
        $seatLock->execute(['org_user_create_' . (int) $current_org_id]);

        // تعداد کاربران فعلی سازمان
        // فقط کاربران حذف‌نشده شمرده شوند — هم‌راستا با api/admin/check-user-limit.php.
        // قبلا کاربران soft-delete شده هم در سقف اشتراک حساب می‌شدند.
    ...
    // نام نباید کاراکترهای خطرناک HTML/کنترلی داشته باشد (جلوگیری از تزریق
    // در جاهایی که بعدا نمایش داده می‌شود). رقم/نقطه/پرانتز مجاز می‌ماند.
