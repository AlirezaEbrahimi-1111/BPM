...
try {
    ...
    $checkAdmin = $db->prepare("SELECT activity_section, organization_id FROM users WHERE id = ?");
    ...
    // ── تغییر: organization_id کاربر جاری ──
    ...
    // اعتبارسنجی ورودی‌ها
    if (empty($input['username']) || empty($input['password']) || empty($input['phone']) || 
