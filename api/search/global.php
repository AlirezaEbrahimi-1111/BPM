...
/**
 * global.php
 * سرچ سراسری زیر هدر — روی تسک‌ها، تیکت‌ها، اطلاعیه‌ها، نوتیفیکیشن‌ها،
 * و فرآیندهای در حال اجرا (workflow) هم‌زمان جستجو می‌کنه، با همون قواعد
 * دسترسی APIهای اصلی هر بخش (quick-search.php برای تسک/تیکت،
 * api/announcements/list.php برای اطلاعیه، Notification.php برای
 * نوتیفیکیشن) تا نتیجه هرگز چیزی بیرون از دسترس کاربر رو نشون نده.
 *
 * تطبیق: کلمه‌به‌کلمه (AND) — هر کلمه‌ی جستجوشده باید جایی در فیلدهای
 * قابل‌جستجوی همون آیتم پیدا بشه، نه اینکه کل عبارت تایپ‌شده باید عینا
 * پشت‌سرهم پیدا بشه.
 *
 * 🔒 اولویت‌بندی نتایج «تسک»: دیگه یه دسته‌ی جداگانه به‌اسم «تاریخچه‌ی
 * تسک» وجود نداره — عنوان/توضیحات/آیتم‌های چک‌لیست/تاریخچه‌ی هر تسک همه
 * زیر همون نتیجه‌ی «کار» جستجو می‌شن، و رتبه‌ی هر کار توسط بالاترین‌
 * اولویت‌ترین لایه‌ای که عبارت جست‌وجو توش پیدا شده تعیین می‌شه: عنوان
...
try {
    ...
    // ───── تسک‌ها ─────
    if (isset($want['task'])) {
        // 🔒 شرط دسترسی باید با taskUserAccess() صفحهٔ جزئیات هم‌راستا باشد؛
        // قبلا فقط creator/assignee/تاریخچه بود، پس کاری که یک مدیر/سرپرست
        ...
        // 🔒 اولویت‌بندی نتایج: هر کاری که کل عبارت جست‌وجو (همه‌ی کلمات، AND)
        // توی «فقط عنوان» پیدا بشه، رتبه‌ی ۱ می‌گیره؛ وگرنه اگه توی «عنوان+
        // توضیحات» پیدا بشه رتبه‌ی ۲؛ وگرنه اگه توی «عنوان+توضیحات+چک‌لیست»
        // پیدا بشه رتبه‌ی ۳؛ وگرنه (یعنی فقط با کمک تاریخچه پیدا شده) رتبه‌ی
        // ۴. چون چک‌لیست/تاریخچه رابطه‌ی یک‌به‌چند با کار دارن (هر کار چند
        // آیتم/رویداد داره)، به‌جای LIKE مستقیم از EXISTS استفاده می‌شه —
        // هر کلمه باید *جایی* (حتی اگه هرکلمه توی یه آیتم/رکورد متفاوت باشه)
        // در دسترس همون لایه پیدا بشه.
        $p1 = [];
        ...
        // ترتیب زیر باید دقیقا با ترتیب ظاهرشدن «?» در متن کوئری یکی
        // باشه: اول CASE (p1,p2,p3) توی SELECT، بعد accessSql، بعد شرط
        // نهایی WHERE (p4)، بعد دوتا idMatch
    ...
    // ───── نوتیفیکیشن‌ها (فقط مال خود کاربر) ─────
