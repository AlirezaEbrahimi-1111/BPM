...
try {
    ...
    // 🔒 قفل: آیتم تیک‌خورده دیگر قابل برداشتن نیست (حتی توسط سازنده یا مسئول)
    ...
    // ── تعیین مسئول این آیتم و کنترل دسترسی تیک‌زدن ──────────────────
    // قانون:
    //   • آیتم دارای ارجاع → فقط مسئولش (کاربر یا اعضای واحد) می‌تواند تیک بزند
    //   • آیتم بدون ارجاع → فقط مسئول فعلی کار (assignee) می‌تواند تیک بزند.
    //     ✅ تعریف‌کننده (creator) پس از واگذاری کار به شخص دیگر، دیگر حق
    //     تیک‌زدن ندارد — فقط می‌تواند آیتم اضافه/حذف کند (add-item.php،
    //     delete-item.php). اگر creator خودش هنوز assignee باشد (کار
    //     واگذار نشده)، همان‌طور که قبلا بود می‌تواند تیک بزند، چون
    ...
    // 🆕 شروع خودکار کار با اولین تیک چک‌لیست — هم برای تسک معمولی، هم مرحلهٔ روتین.
    // چون تیک‌زدن یک آیتم یعنی کاربر عملا شروع به کار کرده، دیگر منطقی نیست دکمهٔ
    // «شروع کار» را همچنان نشان بدهیم. این مسیر همان چیزی است که خود دکمهٔ «شروع کار»
    ...
    // اگر syncTaskStatusWithChecklist (زیر) قرار است همین کار را انجام دهد (تسک معمولی
    ...
    // این دو تابع مستقیما ستون tasks.status را دستکاری می‌کنند بدون این‌که از
    // WorkflowManager::completeStep رد شوند (که workflow_instance_steps را هم به‌روز
    // می‌کند و مرحلهٔ بعدی را فعال می‌کند). بدون این قفل، تیک‌زدن آخرین آیتم چک‌لیست
    // یک مرحلهٔ روتین باعث می‌شد tasks.status مستقیم 'completed' شود ولی
    // workflow_instance_steps همچنان 'active' بماند و کل روتین گیر کند — دقیقا همان
    ...
    // 🔒 هم‌راستا با get.php: مسئول صرف یک/چند آیتم، فقط پیشرفت همان
    // آیتم‌های خودش را ببیند، نه کل چک‌لیست کار را
