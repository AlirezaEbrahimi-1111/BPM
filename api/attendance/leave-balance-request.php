<?php
/**
 * API: درخواست سهمیهٔ تشویقی مرخصی/پاس از سرپرست — یک ردیف ماندگار در
 * leave_bonus_requests ثبت می‌شه (تا با فراموش‌شدن نوتیفیکیشن، درخواست
 * گم نشه) + یک نوتیفیکیشن آنی هم برای اطلاع‌رسانی سریع ارسال می‌شه.
 * خود تأیید/رد درخواست از طریق تب «درخواست‌های سازمان» در
 * attendance_system/pages/requests.php انجام می‌شه (نه pages/users.php —
 * طبق تصمیم صریح، این نوع درخواست باید کنار بقیه‌ی درخواست‌های
 * حضور‌وغیاب/سازمانی باشه، نه توی صفحه‌ی مدیریت کاربران).
...
try {
    ...
    // مقصد درخواست: همیشه سرپرست(های) سازمان — نه مدیر بخش
    ...
    // اگر همین کاربر یک درخواست حل‌نشده از قبل داره، دوباره ردیف جدید نساز
