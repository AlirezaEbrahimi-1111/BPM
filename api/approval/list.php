...
try {
    ...
    $stmt = $db->prepare("
        ...
        JOIN users u ON lr.user_id = u.id AND u.is_active = 1 AND u.is_deleted = 0
    ...
    $stmt = $db->prepare("
        ...
        JOIN users u ON fr.user_id = u.id AND u.is_active = 1 AND u.is_deleted = 0
        WHERE fr.manager_id = ? 
    ...
    if ($user['role'] == 'admin') {
        $stmt = $db->prepare("
            ...
            JOIN users u ON ti.user_id = u.id AND u.is_active = 1 AND u.is_deleted = 0
