 ...
 * جمع‌کردن ۵ فراخوانی جداگانه‌ی داشبورد (my-tasks, delegated-tasks,
 * workflows/list, workflows/active-summary, reports/top-delayed-users)
 * در یک درخواست HTTP واحد — بدون تکرار منطق هرکدوم.
 *
 * هر endpoint دقیقا با همون فایل اصلی خودش (in-process، با
 * output buffering) اجرا می‌شه، پس رفتار و منطق هرکدوم همیشه با
 * نسخه‌ی مستقل خودش (که صفحات دیگه هنوز مستقیم صداش می‌زنن) یکی
 * می‌مونه. هیچ‌کدوم از این ۵ فایل در مسیر موفقیت exit/die صدا
 * نمی‌زنن (فقط در مسیرهای خطا)، پس include‌کردن متوالی‌شون امنه.
 */
...
// از قبل اینجا لود می‌شد ولی پایین‌تر — نیازش داریم تا نطاق پیش‌فرض
// «فعالیت‌های اخیر» (پایین‌تر) رو بر اساس نقش تعیین کنیم
...
// «فعالیت‌های اخیر» در داشبورد باید فقط فعالیت خود کاربر باشه — حتی برای
// مدیر/سوپرادمین — نه کل سازمان (که رفتار پیش‌فرض list.php برای مدیرانه،
// مال صفحه‌ی جداگانه‌ی نظارت کامل workflow-monitor.php).
...
// 🆕 تب «فعالیت‌های اخیر» در داشبورد یه لاگ تاریخچه‌ایه (چه‌کاری/چه‌وقتی)،
// نه لیست فرآیندهای در‌جریان (که همون $recent بالاست و برای ویجت
// گلوگاه‌ها هنوز لازمه) — منبع جدا: api/dashboard/recent-activity.php
// پیش‌فرض نطاق: مدیر/سرپرست → سازمانی (چون سوییچر نطاق فقط توی
// داشبورد مدیر هست)؛ بقیه → شخصی (بدون سوییچری که نشونش بده)
...
// reports/top-delayed-users.php مخصوص کاربرانی‌ست که مجوز
// view_all_org_tasks دارن (مدیران) — requirePermission داخل اون فایل
// در صورت نبود مجوز خودش exit می‌زنه، که include‌شدنش این‌جا کل
// پردازش بوت‌استرپ رو (برای کاربران عادی) قطع می‌کرد. برای همین قبل
// از include، خودمون مجوز رو (بدون exit) چک می‌کنیم.

// 🆕 view_org_dashboard_reports: مجوز محدودتری که فقط همین ویجت‌های
// داشبورد رو سازمانی می‌کنه (بدون دسترسی اضافه به endpointهایی مثل
// api/admin/routines-all.php که پشت همون دو مجوز اصلی قایم شدن)
...
if ($canViewOrgTasks) {
    ...
    // 🆕 «کارهای واگذارشده‌ی تأخیردار» در داشبورد مدیر باید سازمانی باشه،
    // نه فقط کارهایی که خود مدیر شخصا واگذار کرده — چون ممکنه مدیر خودش
    // چیزی واگذار نکرده باشه ولی سازمان پر از کار تأخیردار باشه
    $_GET['scope'] = 'org';
...
// 🆕 «گزارش گلوگاه‌ها» هم همین‌طور — قبلا از لیست شخصی فعالیت‌های
// اخیر (personal_only) ساخته می‌شد که برای مدیر تقریبا همیشه خالی بود
...
// 🆕 تنظیمات روزانه‌ی داشبورد (کارهای ستاره‌دار + تب/فیلتر پیش‌فرض) —
// با CURDATE() سرور خونده می‌شه، نه با مقایسه‌ی تاریخ ساعت سیستم
// کلاینت (localStorage) که کاربرهایی با ساعت سیستم نادرست هیچ‌وقت
