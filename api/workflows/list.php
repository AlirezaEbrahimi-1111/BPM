...
try {
    ...
    // دسترسی: مدیریت/سرپرست همه‌ی روتین‌ها (با هر وضعیتی) را می‌بینند.
    // 🆕 «روتین‌های فعال/جاری» (وضعیت in_progress یا delayed) برایِ *همه‌ی* کاربرانِ
    // سازمان قابل‌نمایش است، نه فقط مدیران و درگیرانِ همان روتین. بقیه‌ی
    // وضعیت‌ها (تکمیل‌شده/لغوشده) مثلِ قبل فقط برایِ درگیران دیده می‌شوند:
    // روتینی که مرحلهٔ فعالش مالِ واحد/خودِ کاربر است، یا خودش سازنده‌اش است.
    ...
    if (!$isManager) {
        ...
        ";
        // 🆕 روتین‌های فعال/جاری (in_progress/delayed) برایِ همه دیده می‌شن؛
        // درگیرانِ یک روتینِ غیرفعال (تکمیل/لغو) همچنان آن را می‌بینند. با
        // personal_only=1 (تب «فعالیت‌های اخیر» شخصیِ داشبورد) فقط شرطِ
        // «درگیر بودن» می‌ماند، مثلِ قبل.
        $visibilityCond = $personalOnly
            ? " AND ( $involvedSql )"
            : " AND ( wi.status IN ('in_progress', 'delayed') OR ( $involvedSql ) )";
