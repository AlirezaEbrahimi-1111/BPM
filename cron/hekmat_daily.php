 ...
 * ارسال روزانه‌ی خودکار حکمت نهج‌البلاغه — ساعت ارسال از صفحه‌ی
 * مدیریت (hekmat_settings) خونده می‌شه، نه از خود crontab. به همین
...
try {
    ...
    if ((int) $settings['is_enabled'] !== 1) {
        // فعال نیست — سکوت (لاگ نمی‌کنه تا فایل لاگ با اجرای هردقیقه‌ای پر نشه)
        exit(0);
    }
    ...
    if ($settings['last_sent_date'] === $today) {
        // امروز قبلا فرستاده شده (مثلا با دکمه‌ی ارسال دستی) — دوباره نفرست
        exit(0);
    }
