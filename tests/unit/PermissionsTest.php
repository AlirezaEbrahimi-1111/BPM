 ...
 *    ۲) اختیار فردی  → اجازهٔ صریح اعطاشده توسط مدیر سازمان
 ...
 * دقیقا همان ساختاری که loadUserForPermissions برمی‌گرداند.
 */
...
return [
    ...
    'tests' => [
        ...
        // 🔒 این تست، دقیقا همان حفره‌ای را می‌بندد که پیدا شد
        'واحد سازمانی نباید دسترسی بدهد' => function (Assert $a) {
            // کارمندی در واحد «مدیریت» — که قبلا به‌اشتباه
        ...
        'سرپرست در هر واحدی، دسترسی مدیریتی دارد' => function (Assert $a) {
            // سرپرست واحد شبکه‌های اجتماعی — که قبلا به‌اشتباه
            ...
            $a->true(
                ...
                '🔓 واحد نباید مانع اختیار نقش شود'
            );
        },
        ...
        'اختیار فردی جزئی، فقط همان یک اجازه را می‌دهد' => function (Assert $a) {
        ...
        'کاربر تهی، هیچ اجازه‌ای ندارد' => function (Assert $a) {
            ...
            $a->false(isSuperAdmin(null), 'سوپرادمین بودن null');
            $a->false(isSameOrganization(null, 1), 'هم‌سازمانی null');
        },
