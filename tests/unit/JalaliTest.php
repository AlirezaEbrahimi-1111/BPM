...
/**
 * ═══════════════════════════════════════════════════════════════════
 *  تست واحد: تبدیلِ تاریخِ میلادی ↔ شمسی
 *  محل: /tests/unit/JalaliTest.php
 * ───────────────────────────────────────────────────────────────────
 *  چرا؟
 *    در پروژه چند پیاده‌سازیِ موازیِ تبدیلِ تاریخ هست
 *    (includes/JalaliHelper.php ، assets/jdf.php ،
 *     attendance_system/jalalidate.php ، و تبدیل‌های درون‌خطی در صفحات).
 *    الگوریتم‌های متفاوت = خطرِ اختلافِ یک‌روزه/کبیسه بینِ صفحه‌ها.
 *
 *  این تست، جفتِ «canonical» را قفل می‌کند:
 *      includes/JalaliHelper.php  ≡  assets/jdf.php
 *    و اگر روزی یکی از این دو تغییرِ رفتار بدهد، همین‌جا لو می‌رود.
 *
 *  توجه: attendance_system/jalalidate.php عمداً این‌جا محکِ تطبیق نمی‌خورد
 *  (نیازِ به بررسیِ جداگانه دارد — به docs بخشِ «بدهی فنی» مراجعه شود).
 *
 *  لنگرهای مرجع (تأییدشده):
 *    ۲۰۲۱-۰۳-۲۱ → ۱۴۰۰/۰۱/۰۱  (نوروزِ ۱۴۰۰)
 *    ۲۰۲۴-۰۳-۱۹ → ۱۴۰۲/۱۲/۲۹  (آخرین روزِ اسفندِ سالِ غیرکبیسه)
 *    ۲۰۲۴-۰۳-۲۰ → ۱۴۰۳/۰۱/۰۱  (نوروزِ ۱۴۰۳)
...
/** خروجیِ JalaliHelper را به رشته‌ی «YYYY/MM/DD» می‌کند. */
...
/** خروجیِ jdf.php را (که بدونِ صفرِ ابتدایی است) هم‌قالبِ بالا می‌کند. */
...
return [
    ...
    'name' => 'تبدیلِ تاریخِ میلادی ↔ شمسی',

    'tests' => [

        // ═══════ لنگرهای تأییدشده ═══════

        'نوروزِ ۱۴۰۰ = ۲۰۲۱-۰۳-۲۱' => function (Assert $a) {
            $a->equals('1400/01/01', jt_helper('2021-03-21'), 'JalaliHelper نوروزِ ۱۴۰۰ را غلط داد');
        },

        'نوروزِ ۱۴۰۳ = ۲۰۲۴-۰۳-۲۰' => function (Assert $a) {
            $a->equals('1403/01/01', jt_helper('2024-03-20'));
        },

        'آخرین روزِ اسفندِ ۱۴۰۲ (غیرکبیسه) = ۲۰۲۴-۰۳-۱۹' => function (Assert $a) {
        ...
        'روزِ بعد از ۱۴۰۲/۱۲/۲۹ می‌شود ۱۴۰۳/۰۱/۰۱' => function (Assert $a) {
            $a->equals('1402/12/29', jt_helper('2024-03-19'));
            $a->equals('1403/01/01', jt_helper('2024-03-20'));
        },
    ],
