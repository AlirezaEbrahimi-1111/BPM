...
if (!function_exists('jalaliToGregorian')) {
    /**
     * ⚠️ نسخهٔ قبلی این تابع خراب بود: در ساخت $days هیچ ترمی برای «سال»
     *    نداشت (فقط ماه و روز داخل سال را می‌شمرد) و مبدأ را ثابت روی ۱۶۰۰
     *    می‌گذاشت، پس خروجی همیشه در حدود سال ۱۶۰۰ میلادی می‌افتاد.
     *    این باعث می‌شد $end_of_month در requests.php و بازهٔ سقف ماهانهٔ پاس
     *    در create.php به تاریخ ۱۶۰۰ برود و کوئری‌های «بین دو تاریخ» صفر
     *    ردیف برگردانند.
     *
     *    اکنون همان الگوریتم استاندارد jdf (هم‌سان با jalaliToGregorianCalc
...
if (!function_exists('getShiftShortageSlots')) {
    function getShiftShortageSlots($check_in, $check_out, $shift_start, $shift_end)
    {
        ...
        // 🔒 اگر خروج قبل از شروع شیفت باشد (مثلا کاربر زودتر از شیفت وارد و
        // به‌اشتباه/زود خارج شده)، بازه نباید از خود خروج شروع شود — از
        // شروع شیفت شروع می‌شود، وگرنه دقایق قبل از شیفت هم به‌غلط کسری
        // حساب می‌شوند (مثلا ورود ۸:۵۵ + خروج ۸:۵۸ برای شیفت ۹:۰۰-۱۴:۰۰
