# استقرار go-api روی سرور (یک‌بار)

الگو دقیقا مثل `crm-service` است، فقط پورت/نام/مسیر فرق دارد. **تا این مراحل انجام
نشده، سرویس روی سرور کاری نمی‌کند و کد `go-api/` صرفا بی‌اثر روی دیسک می‌ماند.**

پیش‌فرض‌ها: پورت داخلی `8091`، مسیر نصب `/opt/bpm-go-api`، کاربر `www-data`.

---

## ۱) کاربر محدود دیتابیس
...
## ۲) ساخت باینری (روی سرور — ساده‌ترین راه، مثل crm-service)
...
```
پر کن:
- `db_name` = نام دیتابیس پروداکشن
- `db_user` = `go_api` ، `db_pass` = همان رمز مرحلهٔ ۱
- `jwt_secret` = **عینا** همان مقدار `/var/www/itmalek/config/config.php`
...
`deploy/apache-go-api-proxy.conf` را داخل `<VirtualHost *:443>`
`/etc/apache2/sites-available/itmalek.conf` (کنار بلوک `/crm/api/`) بگذار.
...
## ۶) اتصال به CI (بعد از این‌که مراحل ۱–۵ کار کردند)

### ۶الف) نصب یک‌بارهٔ ابزار استقرار خودکار روی سرور
...
سه چیز اضافه می‌شود، دقیقا مثل `crm-service`:
...
3. در مرحلهٔ آخر ssh، یک خط: `sudo /opt/bpm-go-api/apply-go-api.sh`

> تا وقتی ۶الف انجام نشده، **این خط را به deploy.yml اضافه نکن** — وگرنه دیپلوی بعدی
...
سه خط `ProxyPass /go/api/` را در vhost کامنت کن + `systemctl reload apache2`.
