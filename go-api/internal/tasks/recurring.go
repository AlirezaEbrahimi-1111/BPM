...
// MaybeStartNextPeriod — پورت دقیق maybeStartNextPeriod() در
// includes/recurring-helper.php: اگر کار تکرارشونده‌ای در period_done
// باشد و دوره‌ی بعدی‌اش شروع شده باشد، وضعیت را به not_started برمی‌گرداند
// (بدون ثبت رکورد تاریخچه — عمدا، چون این عمل سیستمی نباید به کاربر
// نشان داده شود). این عملیات UPDATE می‌نویسد — چون خود منطق «همین که
// تسک دیده شد، دوره را چک کن» طراحی شده (مثل overview.php)، نه چون این
// endpoint قرار است نوشتنی باشد؛ رفع ناهماهنگی صفحات مختلف (که هرکدوم
// این چک را انجام می‌دادند یا نمی‌دادند) با تصمیم صریح انجام شد.
