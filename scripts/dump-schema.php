<?php
/**
 * scripts/dump-schema.php — خروجیِ نرمال‌شده‌ی کلِ اسکیمای دیتابیس، برایِ
 * مقایسه‌ی لوکال ⇄ پروداکشن (رفعِ همون کلاس‌باگی که چند بار این پروژه رو
 * گزیده: یک GRANT یا ستون که فقط رویِ یکی از دو محیط اجرا شده).
 *
 * روی هر دو محیط اجرا کن، خروجی رو با diff مقایسه کن:
 *   php scripts/dump-schema.php > schema-local.txt      (این ماشین)
 *   php scripts/dump-schema.php > schema-prod.txt       (روی سرور، از راهِ SSH)
 *   diff schema-local.txt schema-prod.txt
 *
 * عمداً از mysqldump --no-data استفاده نمی‌کنیم — خروجیِ اون شاملِ
 * AUTO_INCREMENT فعلی/کامنت‌هایی می‌شه که بینِ دو محیط همیشه فرق می‌کنن و
 * دیف رو با نویزِ بی‌ربط پر می‌کنن. این‌جا فقط ستون‌ها، تایپ‌ها، ایندکس‌ها،
 * و GRANTهایِ کاربرِ دیتابیس رو می‌گیریم — دقیقاً چیزی که واقعاً اهمیت داره.
 */
...
// ── GRANTهایِ کاربرِ دیتابیسِ همین اپ — دقیقاً همون کلاس‌باگی که یک بار
