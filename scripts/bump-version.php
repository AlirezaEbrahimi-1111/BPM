<?php
/**
 * scripts/bump-version.php — یک قدم به‌جای سه‌تا برای بامپ نسخه‌ی فوتر.
 *
 * قبلا هر بار: 1) ساعت شمسی فعلی رو با یک دستور inline PHP حساب می‌کردم،
 * 2) عدد نسخه‌ی فعلی رو از footer.php می‌خوندم، 3) هردو رو دستی ادیت می‌کردم.
 * این اسکریپت همون سه قدم رو خودکار می‌کنه — فقط یک اجرا، هم زمان و هم
 * ریسک تایپوی دستی رو حذف می‌کنه.
 *
...
if (!preg_match('/نسخه:\s*([۰-۹.]+)/u', $content, $m)) {
    fwrite(STDERR, "الگوی نسخه در footer.php پیدا نشد\n");
    exit(1);
}
