 ...
 *  اسکیما (بدون FK به users/tasks و ...) تا این ویژگی کاملا ایزوله
...
return [
    ...
    'up' => function (PDO $db) {
        ...
        // 🔒 تک‌ردیفی (id همیشه ۱) — تنظیمات سراسری این فیچر؛ next_index و
        // last_sent_date وضعیت چرخش روزانه رو نگه می‌دارن تا cron هربار از
        // صفر شروع نکنه و دوباره‌کاری همون روز رو تشخیص بده
        $db->exec("
        ...
        // 🔒 quote_text به‌صورت متن کامل (نه FK به hekmat_quotes) ذخیره می‌شه —
        // چون لیست جملات با paste دسته‌جمعی کاملا جایگزین می‌شه؛ اگه FK
        // می‌بود، تاریخچه‌ی ارسال‌های قدیمی با حذف اون جمله بی‌معنی می‌شد
        $db->exec("
