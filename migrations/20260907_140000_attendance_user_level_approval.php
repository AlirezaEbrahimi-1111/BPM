...
/**
 * ═══════════════════════════════════════════════════════════════════
 *  مهاجرت: تأیید حضور‌وغیاب در سطح «کاربر» (نه «دستگاه»)
 *  تاریخ: ۱۴۰۵/۰۶/۱۶
 * ───────────────────────────────────────────────────────────────────
 *  چرا؟
 *    شناسهٔ دستگاه (fingerprint) یک توکن تصادفی در localStorage/کوکی
 *    مرورگر است. «پاک‌کردن داده‌های سایت» آن را می‌برد و کاربر با
 *    شناسهٔ نو دوباره «در انتظار تأیید سرپرست» می‌شود.
 *
 *    راه حل: تصمیم «تأیید» به‌جای «این مرورگر» به «این کاربر در این
 *    سازمان» چسبانده می‌شود. بعد از یک‌بار تأیید، هر مرورگر/دستگاه از
 *    IP مجاز بدون درخواست دوباره کار می‌کند. fingerprint فقط برای لاگ
 *    می‌ماند؛ دروازهٔ امنیتی همان IP شبکهٔ سازمان است (بدون تغییر).
 *
 *  up:
 *    ۱) جدول attendance_approved_users
 *    ۲) backfill از دستگاه‌های approved موجود + از رکوردهای حضور موجود
 *       (تا هیچ کاربر فعالی با اجرای این مهاجرت «قفل» نشود).
 * ═══════════════════════════════════════════════════════════════════
 */
...
return [
    ...
    'up' => function (PDO $db) {
        ...
        // backfill ۱: هر کاربری که دستگاه approved دارد
        try {
            $db->exec("
        ...
        // backfill ۲: هر کاربری که تا حالا حضور ثبت کرده (رفتار فعلی register.php)
        try {
            $db->exec("
                INSERT IGNORE INTO `attendance_approved_users` (organization_id, user_id, approved_at)
