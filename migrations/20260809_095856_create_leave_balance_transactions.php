<?php
/**
 * دفترکل تراکنشی سهمیهٔ مرخصی استحقاقی:
 *   - هر ردیف یک رویداد (تعلق ماهانه، اعطای تشویقی مدیر، کسر بابت مرخصی، اصلاح دستی)
 *   - موجودی فعلی هر کاربر = مجموع amount تمام ردیف‌هاش (نه یک عدد خام قابل‌بازنویسی)
 *   - این‌طوری هم انتقال خودکار سهمیهٔ استفاده‌نشده به ماه بعد جواب می‌ده (چیزی reset نمی‌شه)،
 *     هم تاریخچه برای مدیر/کارمند قابل‌نمایشه
 */
    ...
    'up' => function (PDO $db) {
        $db->exec("
            CREATE TABLE IF NOT EXISTS `leave_balance_transactions` (
                ...
                `amount` DECIMAL(5,2) NOT NULL COMMENT 'مثبت = افزایش موجودی، منفی = کسر',
                `related_leave_request_id` INT NULL,
                `granted_by` INT NULL COMMENT 'برای bonus_grant/manual_adjustment: چه کسی ثبت کرد',
                `note` VARCHAR(255) NULL,
                `period_ym` VARCHAR(7) NULL COMMENT 'برای monthly_accrual: ماه مربوطه به‌شکل YYYY-MM، برای جلوگیری از تعلق تکراری',
