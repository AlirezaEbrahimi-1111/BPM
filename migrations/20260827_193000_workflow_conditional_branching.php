...
/**
 * ═══════════════════════════════════════════════════════════════════
 *  مهاجرت: زیرساخت «شرطی‌سازی مراحل کارهای روتین» (فاز ۱ — فقط اسکیما)
 *  تاریخ: ۱۴۰۵/۰۶/۰۵
 * ───────────────────────────────────────────────────────────────────
 *  هدف: امکان تعریف «مرحلهٔ تصمیم» در قالب روتین که در صورت تأیید به
 *  یک مرحله و در صورت رد به مرحله‌ای دیگر (یا بازگشت به تعریف‌کنندهٔ
 *  نمونه) پرش کند. منطق موتور و رابط کاربری در فازهای بعدی.
 *
 *  تغییرات:
 *   ۱) workflow_steps:
 ...
 *   ۲) workflow_instance_steps.status: افزودن مقدار 'dormant'
 ...
 *        (برای «ساخت تنبل تسک» در فاز ۲ — مراحل خفته task_id ندارند)
 *
 *  همه‌چیز idempotent و امن در برابر اجرا دوباره است.
 * ═══════════════════════════════════════════════════════════════════
 */
...
return [
    ...
    'up' => function (PDO $db) {
        ...
        // ── ۱) ستون‌های جدید workflow_steps ─────────────────────────────
        ...
        if (!$colExists('workflow_steps', 'on_approve_step_order')) {
            $db->exec("ALTER TABLE `workflow_steps`
                ...
                COMMENT 'در صورت تأیید → این step_order؛ NULL = مرحلهٔ بعدی خطی'");
        }
        if (!$colExists('workflow_steps', 'on_reject_mode')) {
            $db->exec("ALTER TABLE `workflow_steps`
                ADD COLUMN `on_reject_mode` ENUM('step','creator') NULL DEFAULT NULL
                COMMENT 'رفتار رد: پرش به یک مرحله، یا بازگشت به تعریف‌کنندهٔ نمونه'");
        }
        ...
        // مقادیر یک تعریف enum را به آرایه تبدیل می‌کند (با پشتیبانی از '' داخل مقدار)
        ...
        // بند DEFAULT را برای MODIFY COLUMN می‌سازد.
        // MariaDB 10.2.7+ مقدار پیش‌فرض رشته‌ای را از قبل با کوتیشن برمی‌گرداند
        // (مثلا "'not_started'")؛ نسخه‌های قدیمی‌تر خام. کوتیت‌کردن دوباره باعث
        ...
        $defaultClause = function ($rawDefault, string $isNullable) use ($db): string {
            ...
            if (strlen($t) >= 2 && $t[0] === "'" && $t[strlen($t) - 1] === "'") {
                return ' DEFAULT ' . $t;            // از قبل یک لیترال SQL است
            }
            return ' DEFAULT ' . $db->quote($t);   // مقدار خام → کوتیت کن
        };

        // ── ۲) افزودن مقدار 'dormant' به enum ستون status ─────────────
        ...
        // اگر status از نوع enum نبود (varchar/text)، هیچ کاری لازم نیست.

        // ── ۳) nullable کردن task_id (برای ساخت تنبل تسک در فاز ۲) ─────
    ...
    'down' => function (PDO $db) {
        ...
        // ۳') برگرداندن task_id به NOT NULL — فقط اگر هیچ ردیف NULL نمانده باشد
        $ti = $colInfo('workflow_instance_steps', 'task_id');
        if ($ti && $ti['IS_NULLABLE'] === 'YES') {
            ...
            if ($nulls === 0) {
                ...
                error_log("workflow branching down(): task_id nullable نگه داشته شد چون $nulls ردیف NULL دارد");
            }
        }

        // ۲') حذف 'dormant' از enum ستون status
        $st = $colInfo('workflow_instance_steps', 'status');
        $vals = $st ? $enumVals($st['COLUMN_TYPE']) : null;
        if ($vals !== null) {
            if (in_array('dormant', $vals, true)) {
