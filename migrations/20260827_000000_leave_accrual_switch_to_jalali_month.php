...
/**
 * ═══════════════════════════════════════════════════════════════════
 *  مهاجرت: انتقال «تعلق ماهانهٔ سهمیهٔ مرخصی» از ماه میلادی به ماه شمسی
 *  تاریخ: ۱۴۰۵/۰۶/۰۵
 * ───────────────────────────────────────────────────────────────────
 *  مشکل:
 *    ensureMonthlyLeaveAccrual() ماه جاری را با date('Y-m') (میلادی) می‌گرفت
 *    و در ستون period_ym ذخیره می‌کرد. در نتیجه سهمیهٔ ۲ روز، اول هر ماه
 *    میلادی اضافه می‌شد نه اول ماه شمسی؛ و ماه شمسی جاری (شهریور ۱۴۰۵)
 *    برای هیچ‌کس تعلق نگرفته بود (چون هنوز ماه میلادی بعدی شروع نشده).
 *
 *  این مهاجرت:
 *    ۱) period_ym همهٔ ردیف‌های monthly_accrual را از «ماه میلادی created_at»
 *       به «ماه شمسی همان تاریخ» بازنویسی می‌کند (amount دست‌نخورده می‌ماند).
 *    ۲) برای هر کاربری که حداقل یک تعلق ماهانه دارد، ماه‌های شمسی جا‌افتاده
 *       تا ماه شمسی جاری را می‌سازد:
 *          amount = round(۲ × daily_work_hours خود فرد × ۶۰)  [دقیقه]
 *
 *  دامنه: همهٔ کاربران، همهٔ سازمان‌ها. کاملا per-user است و هیچ کوئری
 *  بین‌سازمانی ندارد؛ سهمیهٔ سازمان‌ها با هم قاطی نمی‌شود.
 *
 *  همراه این مهاجرت، منطق ensureMonthlyLeaveAccrual() هم به ماه شمسی
...
/** یادداشت ثابت ردیف‌های جبرانی — برای شناسایی آن‌ها در down() */
if (!defined('LBT_JALALI_BACKFILL_NOTE')) {
    define('LBT_JALALI_BACKFILL_NOTE', 'تعلق خودکار ماهانه (جبرانی انتقال به ماه شمسی)');
}

if (!function_exists('lbt_jalali_key')) {
    /** کلید ماه شمسی یک تاریخ میلادی (YYYY-MM-DD) → "YYYY-MM" */
...
if (!function_exists('lbt_next_jalali_key')) {
    /** ماه شمسی بعدی برای کلید "YYYY-MM" */
...
return [
    ...
    'up' => function (PDO $db) {
        ...
        try {
            // ── ۱) بازنویسی period_ym از میلادی به شمسی ──────────────────
            //     فقط ردیف‌هایی که هنوز کلید میلادی دارند (سال ۲۰xx) —
            //     تا اجرا دوباره (در صورت نیاز) به مقادیر شمسی دست نزند.
            $rows = $db->query("
            ...
            // ── ۲) جبران ماه‌های شمسی جا‌افتاده برای هر کاربر ─────────────
            ...
            foreach ($users as $u) {
                ...
                while ($cursor < $currentKey && ++$guard < 240) {
                    ...
                    if ($exists->fetchColumn()) continue; // قبلا هست — رد شو
    ...
    /**
     * بازگرداندن:
     *   - ردیف‌های جبرانی که در up ساخته شدند حذف می‌شوند (با یادداشت ثابت).
     *   - period_ym باقی‌مانده به ماه میلادی created_at برمی‌گردد — که همان
     *     مقدار اولیه است، چون در کد قدیمی period_ym = date('Y-m') در همان
     *     لحظهٔ درج (= ماه created_at) ثبت می‌شد.
     */
